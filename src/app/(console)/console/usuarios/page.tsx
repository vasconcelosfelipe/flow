...
import { listarConvitesPendentes, listarEmpresasConsole, listarUsuariosConsole } from "@/services/console";

export default async function ConsoleUsuariosPage() {
  const [usuarios, empresas, convites] = await Promise.all([
  ...
  return (
    <Container className="space-y-4 pt-5">
      ...
      <GerenciadorUsuariosConsole inicial={usuarios} empresas={empresas} convitesIniciais={convites} />
    </Container>
  );
}
