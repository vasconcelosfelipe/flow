...
/**
 * Só aparece quando há mais de uma empresa — com uma só, entrar já resolve
 * o contexto e esta tela seria um clique a mais sem decisão nenhuma.
 */
export default function SelecionarEmpresaPage() {
  const router = useRouter();
  const { empresas } = SESSAO_MOCK;

  return (
    <div>
      <h1 className="text-titulo font-semibold text-night-text">Escolha uma empresa</h1>
      <p className="mt-1 text-micro text-night-muted">Você tem acesso a {empresas.length} empresas.</p>
        ...
        {empresas.map((empresa) => (
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="block truncate text-nano text-ink-muted">{ROTULO_PAPEL[empresa.papel]}</span>
            </span>
