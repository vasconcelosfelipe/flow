...
/**
 * Mesma lógica de ocorrência (única/parcelado/recorrente) de
 * `BotaoNovaPendencia`, com duas diferenças: a conta já vem fixa (esta
 * tela é a única porta de entrada pra lançar num cartão) e o resultado no
 * servidor não é uma pendência — `criarPendencia` reconhece conta tipo
 * CARTAO e grava `status: PAGO` com a data calculada pelo ciclo da fatura
 * em vez de `PENDENTE`. O campo que aqui chama "Data da compra" é o mesmo
 * `dataVencimento` que na pendência normal seria o vencimento — o servidor
 * decide o que fazer com ele a partir do tipo da conta.
 */
